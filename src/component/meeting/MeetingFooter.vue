...
<template>
  <footer
    class="relative z-30 flex h-14 sm:h-20 items-center justify-between overflow-x-auto overflow-y-visible border-t border-white/10 bg-[#1a1a1a] px-2 sm:px-8 gap-2 scrollbar-none w-full">
    <div class="flex shrink-0 items-center space-x-1 sm:space-x-4">
      <div class="flex items-center overflow-hidden rounded-lg">
        ...
        <el-popover trigger="click" placement="top" :width="'min(260px, 85vw)'" teleported>
          <template #reference>
      ...
      <div class="flex items-center overflow-hidden rounded-lg">
        ...
        <el-popover trigger="click" placement="top" :width="'min(256px, 85vw)'" teleported>
          <template #reference>
    ...
    <div class="flex shrink-0 items-center space-x-1 sm:space-x-4">
      <button class="flex flex-col items-center" type="button" @click="emit('openMembers')">
        <div
          class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-white/10 text-white transition-all hover:bg-blue-600/30">
          <el-icon :size="20" class="sm:text-[24px]">
            <Avatar />
          </el-icon>
        </div>
        <span class="mt-1 hidden sm:block text-xs text-slate-300">成员</span>
      </button>

      <button class="flex flex-col items-center" type="button" @click="emit('openChat')">
        <div
          class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-white/10 text-white transition-all hover:bg-slate-100/15">
          <el-icon :size="20" class="sm:text-[24px]">
            <ChatDotRound />
          </el-icon>
        </div>
        <span class="mt-1 hidden sm:block text-xs text-slate-300">聊天</span>
      </button>

      <button class="flex flex-col items-center" type="button" @click="emit('toggleShare')">
        <div class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl text-white transition-all"
          :class="props.isScreenSharing ? 'bg-blue-600' : 'bg-white/10 hover:bg-blue-600'">
          <el-icon :size="20" class="sm:text-[24px]">
            <Monitor />
          </el-icon>
        </div>
        <span class="mt-1 hidden sm:block text-xs text-slate-300">共享屏幕</span>
      </button>
      ...
      <button v-if="props.isHost" class="flex flex-col items-center" type="button" @click="emit('toggleRecord')">
        <div class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl text-white transition-all"
          ...
          <svg aria-hidden="true" class="h-5 w-5 sm:h-6 sm:w-6" viewBox="0 0 24 24" fill="currentColor">
            <circle cx="12" cy="12" r="4.5" />
            <path
              d="M12 2.75a9.25 9.25 0 1 0 9.25 9.25A9.26 9.26 0 0 0 12 2.75m0 1.5a7.75 7.75 0 1 1-7.75 7.75A7.76 7.76 0 0 1 12 4.25" />
          </svg>
        </div>
        <span class="mt-1 hidden sm:block text-xs text-slate-300">录制</span>
      </button>

      <button class="flex flex-col items-center" type="button">
        <div
          class="flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-white/10 text-white transition-all hover:bg-white/20">
          <el-icon :size="20" class="sm:text-[24px]">
            <MoreFilled />
          </el-icon>
        </div>
        <span class="mt-1 hidden sm:block text-xs text-slate-300">更多</span>
      </button>
    </div>

    <div class="flex shrink-0 items-center justify-end">
      <button
        class="flex items-center rounded-lg bg-red-600 px-3 sm:px-6 py-1.5 sm:py-2.5 text-sm sm:text-base font-bold text-white transition-all hover:bg-red-700"
        type="button" @click="emit('leaveMeeting')">
        <el-icon class="sm:mr-2" :size="18">
          <PhoneFilled />
        </el-icon>
        <span class="hidden sm:inline">{{ props.isHost ? '结束会议' : '退出会议' }}</span>
      </button>
    </div>
  </footer>
</template>
...
.footer-toggle {
  @apply flex h-10 sm:h-12 items-center justify-center transition-all;
}

.footer-toggle--left {
  @apply w-10 sm:w-12 rounded-none;
}

.footer-toggle--right {
  @apply w-6 sm:w-8 border-l border-white/10 bg-white/5 text-white hover:bg-white/15;
}
